...
The LLM-based grading pipeline is implemented in the CLI/core code. The API currently uses a rule-based grading stub for user-facing flows.
...
- Uses sample report data (static).
- Supports CSV and PDF export for the displayed metrics.
...
- API grading uses `score_answer()` (rule-based). It does not invoke Gemini grading yet.
...
- Frontend evaluation page uses a hard-coded report and is not wired to live API data.
