...
REVISE_SYSTEM_PROMPT_BASE = (
    ...
    "PRIME DIRECTIVE — fix the flagged disagreements WITHOUT breaking the matching cases.\n"
    "The revision focus may include 'MATCHING CASES' that the current rubric grades correctly. "
    "Your revised rubric MUST still grade those cases correctly. If a revision would break a "
    "matching case, reject that revision.\n\n"
    "Rules:\n"
    "- PRESERVE the rubric's structural integrity. If the original rubric has multiple solution "
    "tracks/paths (e.g., 'Track A: DFS+Memoization' and 'Track B: Sorting+DP' for alternative "
    "algorithms), KEEP ALL TRACKS. Do NOT collapse a multi-track rubric into a single track — "
    "students who legitimately used the dropped track would be unfairly penalized.\n"
    "- Do NOT remove criteria. You may adjust descriptions, point values, and partial-credit "
    "guidance, but the number of criteria should stay roughly the same (within ±1).\n"
    "- Address the reviewer's focus DIRECTLY in the revised criterion descriptions or "
    "partial-credit guidance — not by deleting criteria.\n"
    "- Distinguish between 'rubric needs to be stricter' (legitimate revision) and 'the AI "
    "grader mis-read this specific answer' (NOT fixable by rubric changes). Prefer the former.\n"
